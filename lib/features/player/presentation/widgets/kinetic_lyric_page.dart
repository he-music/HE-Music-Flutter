...
import '../../../lyrics/presentation/widgets/kinetic_lyric_rail.dart';

/// Player host for the Kinetic Note lyric visualizer.
class KineticLyricPage extends ConsumerWidget {
  const KineticLyricPage({
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    return _KineticLyricDataHost(
      key: const ValueKey<String>('kinetic-lyric-page'),
      emptyText: emptyText,
      onSeek: onSeek,
      palette:
          palette ??
...
class _KineticLyricDataHost extends ConsumerWidget {
  const _KineticLyricDataHost({
    required this.emptyText,
    required this.onSeek,
    required this.palette,
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return documentAsync.when(
      ...
      error: (error, stackTrace) =>
          _KineticLyricFallback(text: emptyText, palette: palette),
    );
  }
}

class _KineticLyricFallback extends StatelessWidget {
  const _KineticLyricFallback({required this.text, required this.palette});
