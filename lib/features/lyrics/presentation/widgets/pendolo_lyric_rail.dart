...
import 'pendolo_lyric_painter.dart';

// Player-owned resources and subscriptions stay local to the wheel rail.
class PendoloLyricRail extends ConsumerStatefulWidget {
  const PendoloLyricRail({
  ...
  @override
  ConsumerState<PendoloLyricRail> createState() => _PendoloLyricRailState();
}

class _PendoloLyricRailState extends ConsumerState<PendoloLyricRail>
  ...
  Object? _layoutKey;
  List<PendoloPaintRow> _rows = [];
  List<double> _angles = [];
  PendoloLyricPainter? _painter;
  ...
  @override
  void didUpdateWidget(covariant PendoloLyricRail oldWidget) {
  ...
  Widget build(BuildContext context) => LayoutBuilder(
    builder: (context, constraints) {
      ...
      final geometry = PendoloWheelGeometry(size);
      ...
      if (_layoutKey != key) {
        ...
        _rows = entries.map((entry) {
            ...
            final p = TextPainter(
              ...
              maxLines:
                  (size.height *
                      ...
                      .clamp(1, 20),
              ellipsis: '…',
            )..layout(maxWidth: geometry.textWidth);
            return p;
          }
          ...
          final base = paragraph(
            entry.line.text,
            style.copyWith(
              color: widget.palette.foreground.withValues(alpha: opacity),
            ),
          );
        ...
        _owner.retain(_rows);
        _angles = List.filled(_rows.length, 0);
        final focal = _rows.indexWhere((row) => row.entry.index == _anchor);
        if (focal >= 0) {
          for (var i = focal + 1; i < _rows.length; i++) {
            _angles[i] =
                _angles[i - 1] +
                geometry.step(_rows[i - 1].height, _rows[i].height);
          }
      ...
      final painter = PendoloLyricPainter(
        rows: _rows,
        anchor: _anchor,
      ...
      return RepaintBoundary(
        key: const ValueKey('pendolo-lyric-repaint-boundary'),
        child: Semantics(
          label: selected == null
              ? null
          ...
          child: Listener(
            ...
            child: GestureDetector(
              ...
              child: CustomPaint(
                key: const ValueKey('pendolo-lyric-painter'),
                size: size,
                painter: painter,
              ),
            ),
