...
- (void)startVisualizer:(int)captureSize captureRate:(int)captureRate enableWaveform:(BOOL)enableWaveform enableFft:(BOOL)enableFft {
    ...
    if (_tapPlayerItem && _visualizerTapCaptureSize != captureSize) {
        [self releaseTap];
    }
...
- (void)stopVisualizer {
    ...
    _visualizerEnableWaveform = NO;
    _visualizerEnableFft = NO;
    // Keep the pass-through tap attached to the current playing item. Removing
    // its audioMix while AVPlayer is active audibly interrupts playback.
}
...
- (void)ensureTap {
    ...
    if (_tapPlayerItem && item != _tapPlayerItem) {
    ...
    _tapPlayerItem = item;
    _visualizerTapCaptureSize = _visualizerCaptureSize;
}
...
static void processTap(MTAudioProcessingTapRef tap, CMItemCount frameCount, MTAudioProcessingTapFlags flags, AudioBufferList *bufferListInOut, CMItemCount *frameCountOut, MTAudioProcessingTapFlags *flagsOut) {
    ...
    for (UInt32 i = 0; i < captureSize; i++) {
        ...
        int unsignedSample = (int)(sample*128.0*3) + 128;
...
- (BOOL)isVisualizerEnabled {
    return _visualizerEnableWaveform || _visualizerEnableFft;
}
